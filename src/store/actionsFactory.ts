...
export const getMovementsMonth =
	(userUid: string) =>
	async (dispatch: Dispatch): Promise<void> => {
		const movements = await MovementService.getMovementsMonth(userUid)

		dispatch({ type: ActionTypes.GET_MOVEMENTS_MONTH, payload: movements })
	}
